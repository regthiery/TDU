...
v1x = mean_values[1,2]$mean_x
v1y = mean_values[1,3]$mean_y
...
data$vx [data$group==2] = vbx
data$vy [data$group==2] = vby
...
ggplot() +
  ...
  geom_segment(data = data,
               aes(x = Lon, y = Lat, 
                   xend = Lon + vx / 1e2 ,  # Augmenter la longueur de la flèche
                   yend = Lat + vy / 1e2,
...
ggsave("outputs/californie5.png", width = 8, height = 6)
